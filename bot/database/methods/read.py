...
def get_all_unique_items() -> dict[str, str]:
    """Return mapping of display names to one internal name."""
    items = Database().session.query(Goods.name).all()
    result: dict[str, str] = {}
    for name, in items:
...
def get_item_categories(base_name: str) -> list[str]:
    """Return categories containing items with the given base name."""
    categories = (Database().session.query(Goods.category_name)
                  .filter(Goods.name.like(f"{base_name}__%"))).all()
