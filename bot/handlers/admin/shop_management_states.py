...
from bot.keyboards import (shop_management, goods_management, categories_management, back, item_management,
                           ...
                           promo_manage_actions, update_item_options)
...
async def update_item_callback_handler(call: CallbackQuery):
    ...
    role = check_role(user_id)
    if role & Permission.SHOP_MANAGE:
        await bot.edit_message_text('Pasirinkite veiksmą:',
                                    chat_id=call.message.chat.id,
                                    message_id=call.message.message_id,
                                    reply_markup=update_item_options())
        return
    await call.answer('Nepakanka teisių')


async def update_item_edit_callback_handler(call: CallbackQuery):
    bot, user_id = await get_bot_user_ids(call)
...
async def reassign_product_callback_handler(call: CallbackQuery):
    bot, user_id = await get_bot_user_ids(call)
    role = check_role(user_id)
    TgConfig.STATE[f'{user_id}_message_id'] = call.message.message_id
    if role & Permission.SHOP_MANAGE:
        ...
        markup = InlineKeyboardMarkup()
        for _, internal in items.items():
            markup.add(InlineKeyboardButton(display_name(internal),
                                           callback_data=f'reassign_item_{internal}'))
        markup.add(InlineKeyboardButton('🔙 Back', callback_data='update_item'))
        await bot.edit_message_text('Select product:',
                                    chat_id=call.message.chat.id,
                                    message_id=call.message.message_id,
                                    reply_markup=markup)
        return
...
async def reassign_item_selected(call: CallbackQuery):
    bot, user_id = await get_bot_user_ids(call)
    internal = call.data[len('reassign_item_'):]
    base = display_name(internal)
    TgConfig.STATE[f'{user_id}_reassign_item'] = internal
    TgConfig.STATE[f'{user_id}_reassign_base'] = base
    message_id = TgConfig.STATE.get(f'{user_id}_message_id')
    categories = get_all_category_names()
    markup = InlineKeyboardMarkup()
    for cat in categories:
        markup.add(InlineKeyboardButton(cat, callback_data=f'reassign_cat_{cat}'))
    markup.add(InlineKeyboardButton('🔙 Back', callback_data='reassign_product'))
    await bot.edit_message_text('Select category:',
                                chat_id=call.message.chat.id,
                                message_id=message_id,
                                reply_markup=markup)
...
async def reassign_category_selected(call: CallbackQuery):
    bot, user_id = await get_bot_user_ids(call)
    category = call.data[len('reassign_cat_'):]
    base = TgConfig.STATE.get(f'{user_id}_reassign_base')
    message_id = TgConfig.STATE.get(f'{user_id}_message_id')
    current = set(get_item_categories(base))
    subs = get_all_subcategories(category)
    if subs:
        markup = InlineKeyboardMarkup()
        for sub in subs:
            label = f'{sub} ✅' if sub in current else sub
            markup.add(InlineKeyboardButton(label, callback_data=f'reassign_sub_{sub}'))
        markup.add(InlineKeyboardButton('🔙 Back', callback_data='reassign_back_to_cats'))
        await bot.edit_message_text('Select subcategory:',
                                    chat_id=call.message.chat.id,
                                    message_id=message_id,
                                    reply_markup=markup)
    ...
    if category in current:
        await call.answer('Product already in this category')
        return
    markup = InlineKeyboardMarkup().add(
        InlineKeyboardButton('✅ Yes', callback_data=f'reassign_add_{category}'),
        InlineKeyboardButton('❌ No', callback_data='reassign_back_to_cats')
    )
    await bot.edit_message_text(f'Add product to {category}?',
                                chat_id=call.message.chat.id,
                                message_id=message_id,
                                reply_markup=markup)


async def reassign_subcategory_selected(call: CallbackQuery):
    bot, user_id = await get_bot_user_ids(call)
    sub = call.data[len('reassign_sub_'):]
    base = TgConfig.STATE.get(f'{user_id}_reassign_base')
    message_id = TgConfig.STATE.get(f'{user_id}_message_id')
    current = set(get_item_categories(base))
    if sub in current:
        await call.answer('Product already in this subcategory')
        return
    markup = InlineKeyboardMarkup().add(
        InlineKeyboardButton('✅ Yes', callback_data=f'reassign_add_{sub}'),
        InlineKeyboardButton('❌ No', callback_data='reassign_back_to_cats')
    )
    await bot.edit_message_text(f'Add product to {sub}?',
                                chat_id=call.message.chat.id,
                                message_id=message_id,
                                reply_markup=markup)


async def reassign_confirm_add(call: CallbackQuery):
    bot, user_id = await get_bot_user_ids(call)
    sub = call.data[len('reassign_add_'):]
    internal = TgConfig.STATE.get(f'{user_id}_reassign_item')
    base = TgConfig.STATE.get(f'{user_id}_reassign_base')
    message_id = TgConfig.STATE.get(f'{user_id}_message_id')
    item_info = check_item(internal)
    new_name = generate_internal_name(base)
    create_item(new_name, item_info['description'], item_info['price'], sub, item_info.get('delivery_description'))
    markup = InlineKeyboardMarkup().add(
        InlineKeyboardButton('✅ Yes', callback_data='reassign_more_yes'),
        InlineKeyboardButton('❌ No', callback_data='reassign_more_no')
    )
    await bot.edit_message_text('Add this product somewhere else?',
                                chat_id=call.message.chat.id,
                                message_id=message_id,
...
async def reassign_back_to_cats(call: CallbackQuery):
    bot, user_id = await get_bot_user_ids(call)
    message_id = TgConfig.STATE.get(f'{user_id}_message_id')
    categories = get_all_category_names()
    markup = InlineKeyboardMarkup()
...
async def reassign_more_yes(call: CallbackQuery):
    await reassign_back_to_cats(call)


async def reassign_more_no(call: CallbackQuery):
    bot, user_id = await get_bot_user_ids(call)
    message_id = TgConfig.STATE.get(f'{user_id}_message_id')
    TgConfig.STATE.pop(f'{user_id}_reassign_item', None)
    TgConfig.STATE.pop(f'{user_id}_reassign_base', None)
    TgConfig.STATE.pop(f'{user_id}_message_id', None)
    await bot.edit_message_text('✅ Product reassigned',
                                chat_id=call.message.chat.id,
                                message_id=message_id,
                                reply_markup=back('goods_management'))
...
def register_shop_management(dp: Dispatcher) -> None:
    ...
    dp.register_callback_query_handler(update_item_edit_callback_handler,
                                       lambda c: c.data == 'update_item_edit')
    dp.register_callback_query_handler(reassign_product_callback_handler,
                                       lambda c: c.data == 'reassign_product')
    dp.register_callback_query_handler(reassign_item_selected,
                                       lambda c: c.data.startswith('reassign_item_'))
    dp.register_callback_query_handler(reassign_category_selected,
                                       lambda c: c.data.startswith('reassign_cat_'))
    dp.register_callback_query_handler(reassign_subcategory_selected,
                                       lambda c: c.data.startswith('reassign_sub_'))
    dp.register_callback_query_handler(reassign_confirm_add,
                                       lambda c: c.data.startswith('reassign_add_'))
    dp.register_callback_query_handler(reassign_more_yes,
                                       lambda c: c.data == 'reassign_more_yes')
    dp.register_callback_query_handler(reassign_more_no,
                                       lambda c: c.data == 'reassign_more_no')
    dp.register_callback_query_handler(reassign_back_to_cats,
                                       lambda c: c.data == 'reassign_back_to_cats')
